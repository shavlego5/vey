...
(function (root, factory) {
  ...
  function fill(target) {
    ...
    for (var i = 0; i < options.calendars; i++) {
      ...
      if (dom_has_class(root_element, "pmu-view-years")) {
        ...
        header =
          local_date.getFullYear() - 6 + " - " + (local_date.getFullYear() + 5);
      ...
      (function () {
        ...
        instance.appendChild(
          options.instance_content_template(years_elements, "pmu-years")
        );
      })();
      (function () {
        var months_elements = [],
        ...
        for (month = 0; month < 12; ++month) {
          ...
          month_element.textContent =
            options.locales[options.locale].months[month];
        ...
        instance.appendChild(
          options.instance_content_template(months_elements, "pmu-months")
        );
      })();
      (function () {
        var days_elements = [],
        ...
        for (day = 0; day < 42; ++day) {
          ...
          if (from_user.class_name) {
            from_user.class_name
              .split(" ")
              .forEach(dom_add_class.bind(day_element, day_element));
          }
  ...
  function set_date(target, date, current) {
      ...
      if (options.mode !== "single") {
        if (options.date instanceof Array) {
          ...
          if (options.mode === "range") {
            options.date[1] =
              options.date[1] || parse_date(options.date[0], options);
          }
        } else {
    ...
    options.bound.fill();
    if (dom_matches(target, "mode") && options.default_date !== false) {
      var prepared_date = prepare_date(options),
  ...
  function pickmeup_init(target, initial_options) {
    ...
    if (!target.__pickmeup) {
      ...
      for (i in pickmeup_init.defaults) {
        options[i] =
          i in initial_options ? initial_options[i] : pickmeup_init.defaults[i];
      }
      for (i in options) {
      ...
      if (options.flat) {
        ...
        dom_on(target, target, "mode", options.bound.update);
